...
with tab3:
    ...
    st.markdown("Candlestick charts with SMA/EMA and technical indicators powered by Breeze API")
    st.markdown("---")
    
    if not st.session_state.breeze_connected:
        st.error("❌ Breeze API not connected. Please check your API credentials in the code.")
        st.info("Add your BREEZE_API_SECRET to enable chart features.")
        st.stop()
    
    col1, col2 = st.columns([2, 2])
    
    with col1:
        available_stocks = [s for s in FNO_STOCKS if s in BREEZE_STOCK_MAP]
    ...
    with col2:
        period_options = {
            "1 Day": (timedelta(days=1), "1minute"),
            "5 Days": (timedelta(days=5), "5minute"),
            "1 Month": (timedelta(days=30), "1day"),
            "3 Months": (timedelta(days=90), "1day"),
            "6 Months": (timedelta(days=180), "1day")
        }
    ...
    period_delta, interval = period_options[period_choice]
    
    with st.spinner(f"Loading {selected_chart_stock} data..."):
        df = get_historical_data_breeze(
            selected_chart_stock,
            interval=interval,
            from_date=datetime.now() - period_delta
        )
    ...
    if df is not None and not df.empty and len(df) > 0:
        ...
        fig.update_layout(
            title=f"{selected_chart_stock} - Price Chart (Breeze API)",
        ...
        st.error(f"No data available for {selected_chart_stock} from Breeze API.")
        st.info("This could be due to market hours, API limits, or connection issues.") Moving Averages (EMA 9, 20, 50)",
                    xaxis_title="Date",
                    yaxis_title="Price (₹)",
                    height=400
                )
